...
def objective(x):
	"""
	define the x^2 minimization function that
	takes input variables and has an optima at
	f(0, 0) = 0.0.
...
"""
we need to decode the bit strings to numbers prior to 
evaluating each with the objective function.
We can achieve this by first decoding each substring to 
an integer, then scaling the integer to the desired range. 
This will give a vector of values in the range that can 
then be provided to the objective function for evaluation.
The decode() function below implements this, taking the 
bounds of the function, the number of bits per variable, 
and a bitstring as input and returns a list of decoded 
real values.
"""
...
if __name__ == '__main__':
	...
	# (hyperparameter)
	# This means our actual bit string will have (16 * 2) = 32 bits,
	# given the two input variables.
	n_bits = 16
